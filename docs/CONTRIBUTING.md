...
Hello and thank you for considering contributing to **dotify-cli**!
...
In case all you need is an answer to a question, please refrain from opening an issue and instead visit the project's [discussion page](https://github.com/billsioros/dotify-cli/discussions/categories/q-a).
...
[Issues](https://github.com/billsioros/dotify-cli/issues) should be used to report problems with the library or request a new feature or documentation change. When you create a new Issue, a template will be loaded that will guide you through collecting and providing the required information.
...
- Add [unit or integration tests](https://github.com/billsioros/dotify-cli/tree/master/tests) for added or changed functionality.
- Any code related changes should be accompanied by corresponding changes to the project's documentation.
- If your pull request introduces a new feature, the corresponding `README` [section](https://billsioros.github.io/dotify-cli/latest/#features) must be updated to reflect this.
...
- This project only accepts pull requests related to open issues. In case there is no relevant open issue, feel free to [create one](https://github.com/billsioros/dotify-cli/issues/new/choose).
...
6. [Open a PR in our repository](https://github.com/billsioros/dotify-cli/compare) and follow the PR template so that we can efficiently review the changes
...
[pyenv](https://github.com/pyenv/pyenv) is used, in the context of the **dotify-cli** project, in order to determine the project's compatibility with various versions of Python. Installing `pyenv` is not strictly required, but it is **strongly** recommended.

Having installed `pyenv` in the **global** namespace, you may now run the following snippet, in order to install Python 3.7, 3.8 and 3.9, which, at the time of writing this document, are the only Python versions, supported by **dotify-cli**.
...
The project's version number and [Changelog](https://github.com/billsioros/dotify-cli/blob/master/CHANGELOG.md), depend on a consistent commit history. As a result, your commit message's format is extremely important. Before opening a pull request, please make sure that your commits strictly follow the [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/) format].
